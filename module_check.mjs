...
/* ===== Hesabi 1.0.49 Phase 7: Items, Prices, Stock final stability ===== */
let selectedItemIdsForDelete = new Set();
function normalizeItemKey(v){return String(v||'').trim().replace(/\s+/g,' ').toLowerCase();}
function numericValue(id, fallback=0){const el=$(id); const n=Number(el?.value||fallback||0); return Number.isFinite(n)?n:0;}
function fieldValue(id){return String($(id)?.value||'').trim();}
function itemCategories(){
  const set=new Set(['الكل']);
  (cache.items||[]).forEach(i=>{ if(i && i.isDeleted!==true) set.add(itemCategory(i)); });
  return [...set];
}
function itemCategoryOptions(selected='الكل'){
  return itemCategories().map(c=>`<option value="${esc(c)}" ${String(c)===String(selected)?'selected':''}>${esc(c)}</option>`).join('');
}
function getItemsTab(){
  const allowed=['list','add','edit','prices','excel'];
  const v=state.itemsTab||'list'; return allowed.includes(v)?v:'list';
}
function itemsTabsBar(){
  const active=getItemsTab();
  const tabs=[['list','📋','الأصناف'],['add','➕','إضافة'],['edit','✏️','تعديل'],['prices','💰','الأسعار'],['excel','📥','Excel']];
  return `<div class="page-tabs-card items-tabs-card"><div class="page-tabs-bar compact-tabs">${tabs.map(t=>`<button class="page-tab-btn ${active===t[0]?'active':''}" data-items-tab="${t[0]}"><span class="ico">${t[1]}</span><span class="txt">${t[2]}</span></button>`).join('')}</div></div>`;
}
function bindItemsTabs(){document.querySelectorAll('[data-items-tab]').forEach(b=>b.onclick=()=>{state.itemsTab=b.dataset.itemsTab; save(); renderItems();});}
function filteredItemsByCategory(category='الكل'){
  let arr=(cache.items||[]).filter(i=>i && i.isDeleted!==true);
  if(category && category!=='الكل') arr=arr.filter(i=>itemCategory(i)===category);
  return arr.sort((a,b)=>String(a.name||'').localeCompare(String(b.name||''),'ar'));
}
function itemsCategoryFilterHtml(key, selected='الكل'){
  return `<div class="field demand-extra-filter"><label>التصنيف</label><select data-items-category-filter="${esc(key)}">${itemCategoryOptions(selected)}</select></div>`;
}
function bindItemsCategoryFilter(key, rerender){
  document.querySelectorAll(`[data-items-category-filter="${key}"]`).forEach(sel=>sel.onchange=()=>{state[key+'Category']=sel.value||'الكل'; save(); if(typeof rerender==='function')rerender();});
}
...
function itemsBulkActionsHtml(){
  return `<div class="settings-compact-actions"><button class="btn light mini" data-select-visible-items>تحديد المعروض</button><button class="btn light mini" data-clear-selected-items>إلغاء التحديد</button><button class="btn danger mini" data-delete-selected-items>حذف المحدد</button></div>`;
}
function buildItemRows(meta){
  return (meta.visible||[]).map(i=>{
    const stock=Number(i.stock||0), min=Number(i.minStock||0);
    const status=i.isActive===false?'معطل':(i.customerVisible===false?'مخفي عن العميل':(stock<=0?'نافد':(min>0&&stock<=min?'منخفض':'نشط')));
    const cls=i.isActive===false||stock<=0?'rejected':(min>0&&stock<=min?'pending':'approved');
    return `<tr><td><input type="checkbox" data-select-item="${esc(i.id)}" ${selectedItemIdsForDelete.has(i.id)?'checked':''}></td><td class="name"><b>${esc(i.name||'صنف')}</b><div class="muted">${i.barcode?`باركود: ${esc(i.barcode)} | `:''}${i.model?`موديل: ${esc(i.model)} | `:''}${i.customerVisible===false?'مخفي عند العملاء':'ظاهر عند العملاء'}</div></td><td>${esc(itemCategory(i))}</td><td>${esc(i.unit||'حبة')}</td><td>${money(effectiveItemPrice(i,'cash'))}<br><span class="muted">آجل: ${money(effectiveItemPrice(i,'credit'))}</span></td><td><b>${stock}</b><div class="muted">الحد: ${min}</div></td><td><span class="status ${cls}">${status}</span></td><td>${compactActionButtons(`<button class="btn secondary mini" data-edit-item="${esc(i.id)}">تعديل</button><button class="btn warn mini" data-toggle-customer-visible="${esc(i.id)}">${i.customerVisible===false?'إظهار':'إخفاء'}</button><button class="btn danger mini" data-delete-item="${esc(i.id)}">حذف</button>`)}</td></tr>`;
  });
}
function priceEditorRows(meta){
  return (meta.visible||[]).map(i=>`<tr><td class="name"><b>${esc(i.name||'صنف')}</b><div class="muted">${esc(itemCategory(i))}${i.barcode?` | ${esc(i.barcode)}`:''}</div></td><td><input class="table-input" id="priceCash_${esc(i.id)}" type="number" min="0" value="${Number(i.cashPrice||i.price||0)}"></td><td><input class="table-input" id="priceCredit_${esc(i.id)}" type="number" min="0" value="${Number(i.creditPrice||i.price||i.cashPrice||0)}"></td><td><span class="status ${i.customerVisible===false?'rejected':'approved'}">${i.customerVisible===false?'مخفي':'ظاهر'}</span></td><td>${compactActionButtons(`<button class="btn ok mini" data-save-price-item="${esc(i.id)}">حفظ</button><button class="btn warn mini" data-toggle-customer-visible="${esc(i.id)}">${i.customerVisible===false?'إظهار':'إخفاء'}</button>`)}</td></tr>`);
}
function renderEditItemPane(rows, meta, category){
  const selected=(cache.items||[]).find(i=>i.id===state.editItemId);
  const form=selected?`<div class="card advanced-form-card"><div class="row"><h2>تعديل صنف</h2><span class="status approved">${esc(selected.name||'صنف')}</span></div><div class="hesabi-form-grid"><div class="form-section-title">بيانات الصنف</div><div class="field"><label>اسم الصنف</label><input id="editItemName" value="${esc(selected.name||'')}"></div><div class="field"><label>التصنيف</label><input id="editItemCategory" value="${esc(itemCategory(selected))}"></div><div class="field"><label>الوحدة</label><input id="editItemUnit" value="${esc(selected.unit||'حبة')}"></div><div class="field"><label>باركود / كود الصنف</label><input id="editItemBarcode" value="${esc(selected.barcode||'')}"></div><div class="field"><label>ملاحظات</label><textarea id="editItemNotes">${esc(selected.notes||'')}</textarea></div><div class="form-section-title">الأسعار والمخزون</div><div class="grid2"><div class="field"><label>سعر الكاش</label><input id="editItemCashPrice" type="number" min="0" value="${Number(selected.cashPrice||selected.price||0)}"></div><div class="field"><label>سعر الآجل</label><input id="editItemCreditPrice" type="number" min="0" value="${Number(selected.creditPrice||selected.price||selected.cashPrice||0)}"></div></div><div class="grid2"><div class="field"><label>الكمية الحالية</label><input id="editItemStock" type="number" value="${Number(selected.stock||0)}"></div><div class="field"><label>حد التنبيه</label><input id="editItemMinStock" type="number" min="0" value="${Number(selected.minStock||0)}"></div></div><div class="field"><label>ظهور الصنف عند العملاء</label><select id="editItemCustomerVisible"><option value="true" ${selected.customerVisible!==false?'selected':''}>ظاهر</option><option value="false" ${selected.customerVisible===false?'selected':''}>مخفي</option></select></div><div class="actions"><button class="btn ok" id="saveEditItemBtn">حفظ التعديل</button><button class="btn light" id="cancelEditItemBtn">إلغاء</button></div></div></div>`:'';
  return `${form}<div class="card"><div class="row"><h2>البحث عن صنف للتعديل</h2><span class="status approved">لا يتم عرض كل البيانات إلا عند الطلب</span></div>${demandTableCard('itemsEditList','الأصناف القابلة للتعديل',['تحديد','الصنف','التصنيف','الوحدة','كاش/آجل','المخزون','الحالة','إجراء'],rows,meta,'ابحث أو اضغط عرض البيانات لاختيار صنف للتعديل',itemsCategoryFilterHtml('itemsEditList',category))}</div>`;
}
function itemDuplicateExists({name='',barcode='',excludeId=''}){
  const n=normalizeItemKey(name), b=normalizeItemKey(barcode);
  return (cache.items||[]).some(i=>i && i.id!==excludeId && i.isDeleted!==true && ((b && normalizeItemKey(i.barcode)===b) || (n && normalizeItemKey(i.name)===n)));
}
function currentItemDoc(itemId){return doc(db,'shops',state.shopId,'items',itemId);}
async function addItem(){
  try{
    if(!requireTraderAction('إضافة صنف')) return;
    const name=fieldValue('itemName'), category=fieldValue('itemCategory')||'عام', unit=fieldValue('itemUnit')||'حبة', barcode=fieldValue('itemBarcode'), notes=fieldValue('itemNotes');
    ...
    const cashPrice=numericValue('itemCashPrice'), creditPrice=numericValue('itemCreditPrice',cashPrice), stock=numericValue('itemStock'), minStock=numericValue('itemMinStock');
    const now=Date.now();
    const ref=doc(collection(db,'shops',state.shopId,'items'));
    const data={id:ref.id,shopId:state.shopId,name,category,unit,barcode,notes,cashPrice,creditPrice,price:cashPrice,stock,minStock,isActive:true,customerVisible:true,createdAt:serverTimestamp(),createdMs:now,updatedAt:serverTimestamp(),updatedMs:now,createdBy:uid()};
  ...
  }catch(e){msg('تعذر حفظ الصنف: '+friendlyFirestoreError(e),'error');}
}
function selectItemForEdit(itemId){state.editItemId=itemId; state.itemsTab='edit'; save(); renderItems();}
async function saveSelectedItemEdit(){
  try{
    if(!requireTraderAction('تعديل صنف')) return;
    const item=(cache.items||[]).find(i=>i.id===state.editItemId); if(!item){msg('اختر صنفًا للتعديل.','error');return;}
    const name=fieldValue('editItemName'), barcode=fieldValue('editItemBarcode'); if(!name){msg('اسم الصنف مطلوب.','error'); return;}
    if(itemDuplicateExists({name,barcode,excludeId:item.id})){msg('يوجد صنف آخر بنفس الاسم أو الباركود.','error'); return;}
    const oldStock=Number(item.stock||0), newStock=numericValue('editItemStock');
    const data={name,category:fieldValue('editItemCategory')||'عام',unit:fieldValue('editItemUnit')||'حبة',barcode,notes:fieldValue('editItemNotes'),cashPrice:numericValue('editItemCashPrice'),creditPrice:numericValue('editItemCreditPrice'),price:numericValue('editItemCashPrice'),stock:newStock,minStock:numericValue('editItemMinStock'),customerVisible:$('editItemCustomerVisible')?.value!=='false',updatedAt:serverTimestamp(),updatedMs:Date.now(),updatedBy:uid()};
    await updateDoc(currentItemDoc(item.id),data);
    if(newStock!==oldStock){await addDoc(collection(db,'shops',state.shopId,'stockLedger'),{shopId:state.shopId,itemId:item.id,itemName:name,type:'adjustment',qty:newStock-oldStock,qtyChange:newStock-oldStock,beforeQty:oldStock,afterQty:newStock,reason:'تعديل مباشر من بطاقة الصنف',createdAt:serverTimestamp(),createdMs:Date.now(),actorUid:uid(),actorName:actorName()});}
    await addAudit('edit_item',{itemId:item.id,name,oldStock,newStock});
    msg('تم تعديل الصنف بنجاح.','success'); renderItems();
  }catch(e){msg('تعذر تعديل الصنف: '+friendlyFirestoreError(e),'error');}
}
async function saveItemPriceQuick(itemId){
  try{
    ...
    const cash=numericValue('priceCash_'+itemId), credit=numericValue('priceCredit_'+itemId,cash);
    await updateDoc(currentItemDoc(itemId),{cashPrice:cash,creditPrice:credit,price:cash,updatedAt:serverTimestamp(),updatedMs:Date.now(),updatedBy:uid()});
    await addAudit('update_item_price',{itemId,itemName:item.name,cashPrice:cash,creditPrice:credit});
    msg('تم حفظ السعر.','success');
  }catch(e){msg('تعذر حفظ السعر: '+friendlyFirestoreError(e),'error');}
}
async function toggleItemCustomerVisible(itemId){
  try{
    if(!requireTraderAction('إظهار/إخفاء صنف')) return;
    const item=itemById(itemId); if(!item)return;
    const v=item.customerVisible===false;
    await updateDoc(currentItemDoc(itemId),{customerVisible:v,updatedAt:serverTimestamp(),updatedMs:Date.now(),updatedBy:uid()});
    await addAudit('toggle_item_visibility',{itemId,itemName:item.name,customerVisible:v});
    msg(v?'تم إظهار الصنف للعملاء.':'تم إخفاء الصنف عن العملاء.','success'); renderItems();
  }catch(e){msg('تعذر تغيير الظهور: '+friendlyFirestoreError(e),'error');}
}
async function deleteSingleItem(itemId){
  if(!requireTraderAction('حذف صنف')) return;
  const item=itemById(itemId); if(!item)return;
  const ok=await confirmDialog('حذف صنف','سيتم حذف الصنف من قائمة الأصناف، مع بقاء الفواتير والطلبات القديمة محفوظة بأسمائها. هل تريد المتابعة؟','حذف');
  if(!ok)return;
  try{await deleteDoc(currentItemDoc(itemId)); selectedItemIdsForDelete.delete(itemId); await addAudit('delete_item',{itemId,itemName:item.name}); msg('تم حذف الصنف.','success'); renderItems();}catch(e){msg('تعذر حذف الصنف: '+friendlyFirestoreError(e),'error');}
}
async function deleteItemsByIds(ids){
  if(!requireTraderAction('حذف أصناف متعددة')) return;
  ids=(ids||[]).filter(Boolean); if(!ids.length){msg('حدد صنفًا واحدًا على الأقل.','notice');return;}
  const ok=await confirmDialog('حذف الأصناف المحددة',`سيتم حذف ${ids.length} صنف من قائمة الأصناف. لن يتم حذف الفواتير القديمة. هل تريد المتابعة؟`,'حذف المحدد'); if(!ok)return;
  try{const batch=writeBatch(db); ids.forEach(id=>batch.delete(currentItemDoc(id))); await batch.commit(); await addAudit('delete_items_bulk',{count:ids.length,ids}); clearSelectedItems(); msg('تم حذف الأصناف المحددة.','success'); renderItems();}catch(e){msg('تعذر حذف الأصناف: '+friendlyFirestoreError(e),'error');}
}
async function deleteAllItemsAndStock(){
  if(!requireTraderAction('حذف كل الأصناف')) return;
  const ok=await confirmDialog('حذف كل الأصناف','هذه عملية خطيرة. سيتم حذف كل الأصناف الحالية من الكتالوج. يفضل تصدير نسخة احتياطية قبل المتابعة.','حذف الكل'); if(!ok)return;
  try{const snap=await getDocs(collection(db,'shops',state.shopId,'items')); let batch=writeBatch(db), n=0, total=0; for(const d of snap.docs){batch.delete(d.ref); n++; total++; if(n>=450){await batch.commit(); batch=writeBatch(db); n=0;}} if(n)await batch.commit(); await addAudit('delete_all_items',{count:total}); msg(`تم حذف ${total} صنف.`, 'success'); renderItems();}catch(e){msg('تعذر حذف كل الأصناف: '+friendlyFirestoreError(e),'error');}
}
function openStockAdjustmentDialog(itemId){
  const item=itemById(itemId); if(!item){msg('الصنف غير موجود.','error'); return;}
  const back=$('hesabiDialogBackdrop'), ttl=$('hesabiDialogTitle'), icon=$('hesabiDialogIcon'), bdy=$('hesabiDialogBody'), acts=$('hesabiDialogActions');
  if(!back||!bdy||!acts){return;}
  back.className='hesabi-dialog-backdrop dialog-warn'; ttl.textContent='تسوية مخزون'; icon.textContent='🛠️';
  bdy.innerHTML=`<div class="field"><label>الصنف</label><input value="${esc(item.name)}" disabled></div><div class="grid2"><div class="field"><label>الكمية الحالية</label><input value="${Number(item.stock||0)}" disabled></div><div class="field"><label>الكمية الجديدة</label><input id="stockAdjustNewQty" type="number" value="${Number(item.stock||0)}"></div></div><div class="field"><label>سبب الحركة</label><input id="stockAdjustReason" value="تسوية مخزون"></div>`;
  acts.className='hesabi-dialog-actions'; acts.innerHTML='<button class="btn ok" id="saveStockAdjustBtn">حفظ التسوية</button><button class="btn light" id="cancelStockAdjustBtn">إلغاء</button>';
  back.classList.remove('hidden');
  $('cancelStockAdjustBtn').onclick=hideAppDialog;
  $('saveStockAdjustBtn').onclick=()=>adjustStockItem(itemId, Number($('stockAdjustNewQty')?.value||0), String($('stockAdjustReason')?.value||'تسوية مخزون'));
}
window.editStock=openStockAdjustmentDialog;
async function adjustStockItem(itemId,newQty,reason='تسوية مخزون'){
  try{
    if(!requireTraderAction('تسوية المخزون')) return;
    const item=itemById(itemId); if(!item){msg('الصنف غير موجود.','error');return;}
    const before=Number(item.stock||0), after=Number(newQty||0), diff=after-before;
    if(!Number.isFinite(after) || after<0){msg('الكمية الجديدة غير صحيحة.','error');return;}
    await updateDoc(currentItemDoc(itemId),{stock:after,updatedAt:serverTimestamp(),updatedMs:Date.now(),updatedBy:uid()});
    await addDoc(collection(db,'shops',state.shopId,'stockLedger'),{shopId:state.shopId,itemId,itemName:item.name,type:'adjustment',qty:diff,qtyChange:diff,beforeQty:before,afterQty:after,reason,createdAt:serverTimestamp(),createdMs:Date.now(),actorUid:uid(),actorName:actorName()});
    await addAudit('stock_adjustment',{itemId,itemName:item.name,beforeQty:before,afterQty:after,qtyChange:diff,reason});
    hideAppDialog(); msg('تمت تسوية المخزون وتسجيل الحركة.','success'); renderStock();
  }catch(e){msg('تعذر تسوية المخزون: '+friendlyFirestoreError(e),'error');}
}
function bindItemButtons(){/* compatibility hook for older templates */}
