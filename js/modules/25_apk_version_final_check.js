...
(function(){
  ...
  const BUILD_CODE = 86;
  const TARGET_VERSION = "1.0.108";
  const TARGET_BUILD = 108;
  ...
  function apkInstallReadiness(){
    ...
    if(!runtimeState.ok) warnings.push("نسخة runtime أو APP_VERSION لا تطابق 1.0.108 / build 108.");
    if(!updateState.ok) warnings.push("ملف android-update.json المخزن لا يطابق 1.0.108 / build 108.");
